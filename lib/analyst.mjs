...
import { auditCandidateSources } from './researchQuality.mjs';
...
function dailyStateFrom({ matchedProcessId, matchConfidence, scores, triggerCount, corroboratingCount }) {
  ...
  if (!evidenceReady) return 'no_new_global_insight';
  const structuralTurn = materialChange >= 48;
  const trendAdvance = scores.importance >= 60 && scores.novelty >= 45 && materialChange >= 28;
  const weakSignal = scores.evidenceStrength >= 72 && scores.importance >= 55 && scores.novelty >= 65;
  if (!structuralTurn && !trendAdvance && !weakSignal) return 'no_new_global_insight';
...
export function analyzeCandidate(candidate, researchDate, processes = []) {
  ...
  const sourceAudit = auditCandidateSources({ ...candidate, sources }, researchDate);
  const verified = new Set(sourceAudit.validSources.map((source) => source.url));
  const triggerCount = sources.filter((source) => verified.has(source.url) && source.role === 'trigger').length;
  const corroboratingCount = sources.filter((source) => verified.has(source.url) && source.role === 'corroborating').length;
  ...
  return {
    ...
    analysis: {
      ...
      dailyState,
      materialChangeScore,
      publishThresholdMet: sourceAudit.ok && dailyState !== 'no_new_global_insight',
      scoreWasNormalized,
