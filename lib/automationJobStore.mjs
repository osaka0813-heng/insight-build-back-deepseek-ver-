...
function datePath(date) {
  return `${JOB_DIR}/by-date/${date}.json`;
}
...
export async function createAutomationJob(job) {
  ...
  await writePath(
    ...
    `Point to ${job.date} Insight automation ${job.id}`,
  );

  return job;
}

export async function readAutomationJobForDate(date) {
  const pointer = await readPath(datePath(date));
  if (!pointer.value?.jobId) return undefined;
  return readAutomationJob(pointer.value.jobId);
}
