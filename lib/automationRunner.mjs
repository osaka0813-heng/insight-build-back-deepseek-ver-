...
export function resumeFailedCheckpoint(job) {
  ...
  if (stage === 'research' && Number(state.attempts?.research || 0) >= 3) {
    state.writerDraft = buildResearchFailureObservation(
      job.date,
      new Error(state.message || 'Research retries exhausted'),
    );
    state.status = 'checkpointed';
    state.stage = 'publish';
    state.message = '研究重试已用尽，准备发布透明的当日缺失记录';
    job.status = 'running';
    job.currentScope = scope;
    job.currentStage = 'publish';
    job.failedStage = undefined;
    job.completedAt = undefined;
    job.message = '研究重试已用尽，转入公开记录';
    return { scope, stage: 'publish' };
  }
...
export async function recoverStage(job, error) {
  ...
  if (stage === 'research') {
    state.writerDraft = buildResearchFailureObservation(job.date, error);
    state.recoveredStage = stage;
    state.recoveryReason = error instanceof Error ? error.message : String(error);
    state.status = 'checkpointed';
    state.stage = 'publish';
    job.currentStage = 'publish';
    job.message = 'Research 连续失败，已生成透明的当日缺失记录。';
    await saveAutomationJob(job);
    return true;
  }
