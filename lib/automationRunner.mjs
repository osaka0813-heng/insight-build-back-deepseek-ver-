...
export function alignWriterDraftToJobDate(writerDraft, jobDate) {
  if (!writerDraft || !/^\d{4}-\d{2}-\d{2}$/.test(String(jobDate || ''))) {
    return writerDraft;
  }

  return {
    ...writerDraft,
    insight: writerDraft.insight
      ? { ...writerDraft.insight, publishedAt: `${jobDate}T03:00:00.000Z` }
      : writerDraft.insight,
    dailyStateDraft: writerDraft.dailyStateDraft
      ? { ...writerDraft.dailyStateDraft, id: `daily-global-${jobDate}`, date: jobDate }
...
export async function executeOneStage(job) {
  ...
  if (stage === 'research') {
    ...
    // The scheduled edition date is authoritative; a model event date must
    // never shift a daily edition into the previous day.
    state.writerDraft = alignWriterDraftToJobDate(state.writerDraft, job.date);
    if (!state.writerDraft?.qualityChecks?.publishThresholdMet) {
      ...
      state.message = '今日未形成新结构性洞察的编辑结论已发布，保留最近一篇有效 Insight。';
...
export function buildResearchFailureObservation(date, error) {
  const reason = error instanceof Error ? error.message : String(error || 'Research unavailable');
  const id = `writer-research-unavailable-${date}`;
  return {
    id,
    scope: 'global',
    status: 'draft',
    ...
    qualityChecks: {
      ...
      warnings: [`Research did not complete: ${reason.slice(0, 300)}`],
    },
    dailyStateDraft: {
      id: `daily-global-${date}`,
      scope: 'global',
      date,
      ...
      content: {
        en: {
          label: 'No verified daily insight',
          decisionTitle: 'Research could not be completed',
          decisionSummary: 'The independent-source scan did not complete reliably, so no claim is published for this date.',
          thresholdReason: 'Publishing is withheld because the evidence collection stage did not complete.',
          observeNext: ['Retry the independent-source scan on the next scheduled recovery run.'],
        },
        zh: {
          label: '当日无已验证洞察',
          decisionTitle: '研究阶段未能可靠完成',
          decisionSummary: '独立来源扫描未能可靠完成，因此不为该日期发布未经验证的判断。',
          thresholdReason: '证据收集阶段没有完成，系统保留旧洞察并等待恢复。',
          observeNext: ['在下一次定时恢复中重新执行独立来源扫描。'],
        },
      },
    },
  };
