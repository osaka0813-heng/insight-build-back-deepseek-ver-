...
async function request(url, body, timeoutMs = 240_000, maxAttempts = 1) {
  let lastError;
...
export async function deepseekResponsesJSON({
  ...
  const config = requireKey();
  if (!webSearch) {
    return chatJSONFallback({ model, instructions, input, schema,
      maxTokens: Math.min(maxOutputTokens, 16000), timeoutMs: 60000 });
  }
