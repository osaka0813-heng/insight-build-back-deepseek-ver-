...
export function mergeApprovedDraft(content, rawDraft, publishedAt = new Date().toISOString()) {
  ...
  const editionDate = rawDraft.dailyStateDraft?.date;
  const editionPublishedAt = /^\\d{4}-\\d{2}-\\d{2}$/.test(String(editionDate || ''))
    ? `${editionDate}T03:00:00.000Z`
    : publishedAt;

  const writerDraft = {
    ...rawDraft,
    // The candidate/event date belongs in its evidence. The public Insight
    // edition date must be the day the editorial pipeline publishes it.
    insight: {
      ...rawDraft.insight,
      publishedAt: editionPublishedAt,
    ...
    ? (() => {
        ...
        const rejectedContent = Object.fromEntries(
          ['en', 'zh'].map((language) => {
            ...
            {
              ...copy,
              label: language === 'zh' ? '今日未形成新结构性洞察' : 'No new structural insight today',
              decisionTitle: language === 'zh' ? '现有证据不足以支持新的结构性判断' : 'Current evidence does not support a new structural conclusion',
