...
export function buildWriterDraft({ researchDraft, candidate, model, generated, process }) {
  ...
  const sourceAudit = auditCandidateSources(candidate, researchDraft.researchDate || candidate.date);
  const scope = 'global';
  const insightId = generated.insightId || `${scope}-${date}-${slugify(generated.en.cover.title)}`;
  ...
  const processUpdate = processId && generated.processUpdate ? {
    processId,
    scope,
    updatedAt: new Date().toISOString(),
  ...
  if (!sourceAudit.ok) {
    checks.push('Fewer than two independent, current, clickable sources.');
  }
  ...
  return {
    ...
    qualityChecks: {
      publishThresholdMet: Boolean(candidate.analysis?.publishThresholdMet && sourceAudit.ok),
      sourceUrlsPreserved: sourceAudit.ok,
      noMarkdownLinksInCopy: true,
      languagesComplete: Boolean(generated.en && generated.zh),
      warnings: checks,
    },
    insight,
    dailyStateDraft,
